...
with open('preprocessing_pipeline/output_files/PDF/merged_boxes_lp_pdfplumber.json', 'r') as f:
    data_lp_pdfplumber = json.load(f)

# Layoutparser + PyMuPDF
with open('preprocessing_pipeline/output_files/PDF/merged_boxes_lp_pymupdf.json', 'r') as f:
    data_lp_pymupdf = json.load(f)


# Function to group data by page
...
# Group data by page
data_by_page_lp_pdfplumber = group_by_page(data_lp_pdfplumber)
data_by_page_lp_pymupdf = group_by_page(data_lp_pymupdf)
...
# Function to create and save merged output
def create_merged_output(data_by_page, output_name):
    ...
    for page_number, img in enumerate(images, start=1):
        ...
        output_filename = f'preprocessing_pipeline/output_files/PDF/{output_name}_page_{page_number}.pdf'
    ...
    merger.write(f"preprocessing_pipeline/output_files/PDF/{output_name}.pdf")

    # Delete the individual page PDFs
    for page_number in range(1, len(images) + 1):
        os.remove(f'preprocessing_pipeline/output_files/PDF/{output_name}_page_{page_number}.pdf')

    # Close the PdfFileMerger
    merger.close()

# Create and save merged outputs
create_merged_output(data_by_page_lp_pdfplumber, 'merged_output_lp_pdfplumber')
create_merged_output(data_by_page_lp_pymupdf, 'merged_output_lp_pymupdf')
